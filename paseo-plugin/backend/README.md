...
Normal operation uses `server/backend/main.ts` with Node.js 22.14+ and Git. The
Workbench backend does not require a Python executable, package, PyInstaller
artifact or backend download; a Python executable may still be resolved for a
business repository's explicit runtime requirement.
...
`npm test` includes native Node/Git and supervisor process tests. The removed Python
service was compared against Node on real temporary repositories before cleanup;
the evidence is retained in `docs/verification/backend-parity-review-recovery-20260914.md`.
`node scripts/verify-live.mjs` is opt-in and loads this plugin in an actual
...
The project lease uses built-in `node:sqlite` (available in the required Node version), not a native npm addon or a
Python service. Project runtime preparation may still resolve a Python executable when a business repository requests
one. The lease is held for the transport lifetime and released by the OS after a crash, avoiding stale PID-lock deletion
races. Production sockets reject overlong Unix paths rather than allowing truncation. Filesystem deletion completes
before runtime history cleanup; a refused Git deletion preserves the associated sessions and review history.
